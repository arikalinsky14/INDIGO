#!/usr/bin/env bash
#SBATCH --job-name=chroma-lite-eval
#SBATCH --output=job-outputs/slurm-lite-eval.%j.out
#SBATCH --error=job-outputs/slurm-lite-eval.%j.err
...
# ============================================================================
# SLURM Evaluation Script for CHROMA-Lite MLP (evaluate.py)
# ============================================================================
#
# This script evaluates a trained CHROMA-Lite MLP model with two modes:
#
...
#    sbatch pretrain_rgb_to_structure/slurms/slurm_evaluate_lite.sh
#
# 2. Fast evaluation (teacher forcing only):
#    sbatch pretrain_rgb_to_structure/slurms/slurm_evaluate_lite.sh --low-compute
#
# 3. Evaluate with sampling:
#    sbatch pretrain_rgb_to_structure/slurms/slurm_evaluate_lite.sh --sample-predictions
#
# 4. Evaluate with sampling at different temperatures:
#    sbatch pretrain_rgb_to_structure/slurms/slurm_evaluate_lite.sh --sample-predictions --temperature 0.5
#    sbatch pretrain_rgb_to_structure/slurms/slurm_evaluate_lite.sh --sample-predictions --temperature 1.5
#
# ============================================================================
...
echo "============================================================================"
echo "CHROMA-LITE MLP EVALUATION RUN - Job ${SLURM_JOB_ID}"
echo "============================================================================"
echo "PWD:      $(pwd)"
echo "Node:     $(hostname)"
...
D_MODEL="${D_MODEL:-1024}"                  # Hidden layer dimension
N_LAYERS="${N_LAYERS:-8}"                  # Number of hidden layers
DROPOUT="${DROPOUT:-0.1}"                  # Dropout rate

# -------------------- Optimization (used to identify model) --------------------
LR="${LR:-1e-4}"                           # Learning rate (for checkpoint lookup)
BATCH_SIZE="${BATCH_SIZE:-256}"            # Batch size (for checkpoint lookup)
EPOCHS="${EPOCHS:-200}"                      # Epochs (for checkpoint lookup)
...
CMD=(python pretrain_rgb_to_structure/scripts/evaluate.py "${ARGS[@]}" "${USER_ARGS[@]}")
...
#    sbatch pretrain_rgb_to_structure/slurms/slurm_evaluate_lite.sh \
#      --checkpoint pretrain_rgb_to_structure/data/checkpoints/mlp_d256_L4_do0.1_lr0.002_bs256_ep15/latest
#
...
#    sbatch pretrain_rgb_to_structure/slurms/slurm_evaluate_lite.sh --no-optical-sim
#
# 10. SKIP COLOR SWATCH GENERATION:
#    sbatch pretrain_rgb_to_structure/slurms/slurm_evaluate_lite.sh --no-swatch
#
