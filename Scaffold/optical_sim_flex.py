"""
Optical Simulator Adapter for Flexible Materials
================================================

Wraps `src.optical_sim.OpticalSimulator` (or rather, what it should become)
to compute reflectance / sRGB color from user-supplied n,k spectra rather
than from named materials in the JaxLayerLumos library.

This is the only delta needed in the optical-simulation module to support
the flexible-material setup. The original `OpticalSimulator.compute_color`
calls `get_n_k(materials, freq_vector)` which insists on named materials
present in the JLL registry. Here we build the n_matrix ourselves from
MaterialNK objects whose n,k were sampled on the canonical wavelength grid.

The wavelength grid in `src.material_features.CANONICAL_FREQ_HZ` is
defined to match the simulator's grid bit-for-bit, so no interpolation is
needed inside this function.

Usage
-----
    sim = FlexOpticalSimulator(incidence_angle=0)
    rgb = sim.compute_color(pool=[...MaterialNK...],
                             slot_indices=[2, 0, 5],
                             thicknesses_nm=[100, 50, 75])

This module depends on `jaxlayerlumos` only for:
- `stackrt_n_k` (the actual transfer-matrix calculation)
